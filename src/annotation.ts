...
import { color, colorStore, config, onConfigUpdated } from './config'
...
export function RegisterAnnotations(ctx: ExtensionContext) {
  ...
  async function updateDecorations() {
    ...
    const themeKey = Object.keys(colorStore.value)[0]

    const REGEX_FULL = new RegExp(`[^\w\d]((?:${Object.keys(colorStore.value[themeKey]).join('|')}))`, 'g')
    ...
    decorations = (await Promise.all(keys.map(async ([range, key]) => {
      ...
      const svgURI = toDataUrl(getSVG(colorStore.value[themeKey][key]))
      ...
      const item: DecorationMatch = {
        ...
        hoverMessage: await getColorInfoMarkdown(colorStore.value, key as string),
        key,
      }
      return item
    }))).filter(isTruthy)
